...
export function CategoryManager({ categories, permissions }: { categories: CategoryOption[]; permissions: { create: boolean; update: boolean; delete: boolean } }) {
...
export function InventoryItemActions({ product, categories, canDelete }: { product: EditableProduct; categories: CategoryOption[]; canDelete: boolean }) {
    ...
    <>
      ...
      <dialog ref={editorRef} className="m-auto w-[min(660px,calc(100%-32px))] rounded-xl border border-border bg-card p-0 text-foreground shadow-xl backdrop:bg-black/35">
        <form ref={formRef} action={updateProductAction.bind(null, product.id)} className="grid gap-4 p-6 sm:grid-cols-2">
          ...
          <div className="flex flex-wrap justify-between gap-2 sm:col-span-2">{canDelete ? <button type="button" onClick={() => deleteConfirmRef.current?.showModal()} className="h-10 rounded-lg border border-destructive/30 px-4 text-xs font-semibold text-destructive">Delete item</button> : <span /> }<div className="flex gap-2"><button type="button" onClick={() => editorRef.current?.close()} className="h-10 rounded-lg border border-border px-4 text-xs font-semibold">Cancel</button><button type="button" onClick={() => saveConfirmRef.current?.showModal()} className="h-10 rounded-lg bg-primary px-4 text-xs font-semibold text-primary-foreground">Save changes</button></div></div>
        </form>
      </dialog>
