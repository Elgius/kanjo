  ...
  return (
    <PageContainer className="gap-[22px]">
      ...
      <form className="grid gap-2.5 lg:grid-cols-4 xl:grid-cols-8" aria-label="Audit log filters">
        ...
        <Link href="/settings/audit-log" className="flex h-10 items-center justify-center rounded-lg border border-border bg-card px-4 text-xs font-semibold hover:bg-accent">Clear</Link>
      </form>
      ...
      <Surface className="overflow-hidden px-5">
        ...
        <footer className="flex min-h-16 items-center justify-between border-t border-border py-3 text-[11px] text-muted-foreground"><span>Audit records are retained; archiving is handled separately.</span><nav aria-label="Audit pagination" className="flex gap-2">{data.previousCursor ? <Link href={auditHref(filters, "before", data.previousCursor)} className="flex h-9 items-center gap-1 rounded-lg border border-border px-3"><ChevronLeft className="size-3" />Previous</Link> : null}{data.nextCursor ? <Link href={auditHref(filters, "after", data.nextCursor)} className="flex h-9 items-center gap-1 rounded-lg border border-border px-3">Next<ChevronRight className="size-3" /></Link> : null}</nav></footer>
      </Surface>
    </PageContainer>
  );
}
