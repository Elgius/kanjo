...
type ExistingItem = { id: string; name: string; category: string; retailPrice: string; ingredients: Array<{ productId: string; servingMultiplier: number; standalone: boolean }> };

export function MenuItemForm({ registerId, products, categories, item }: { registerId: string; products: IngredientOption[]; categories: CategoryOption[]; item?: ExistingItem }) {
  const [rows, setRows] = useState(item?.ingredients.length ? item.ingredients : [{ productId: "", servingMultiplier: 1, standalone: false }]);
  const action = item ? updateMenuItemAction.bind(null, item.id, registerId) : createMenuItemAction.bind(null, registerId);
  const legacyCategory = item?.category && !categories.some((category) => category.name === item.category)
    ? item.category
    : null;
  return (
    <form action={action} className="grid gap-3 rounded-xl border border-border bg-card p-4 sm:grid-cols-2">
      ...
      <fieldset className="grid gap-2 sm:col-span-2">
        ...
        {rows.map((row, index) => (
          <div key={index} className="grid gap-2 rounded-lg bg-accent p-3 sm:grid-cols-[minmax(0,1fr)_110px_auto]">
            <select name="ingredientProductId" value={row.productId} onChange={(event) => setRows((current) => current.map((value, rowIndex) => rowIndex === index ? { ...value, productId: event.target.value, standalone: false } : value))} className={fieldClass} required>
            ...
            <label className="flex min-h-9 items-center gap-2 text-[11px] sm:col-span-3"><input type="checkbox" name="standaloneProductId" value={row.productId} checked={row.standalone} disabled={!row.productId} onChange={(event) => setRows((current) => current.map((value, rowIndex) => rowIndex === index ? { ...value, standalone: event.target.checked } : value))} className="size-4 accent-primary" /><span>Sell this ingredient as a standalone menu item</span></label>
          </div>
        ))}
