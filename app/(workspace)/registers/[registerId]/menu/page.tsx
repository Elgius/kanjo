...
export default async function RestaurantMenuPage({ params, searchParams }: PageProps<"/registers/[registerId]/menu">) {
  const authorization = await requireCapability("RESTAURANT_MENU_VIEW", "RESTAURANT_MENU_PAGE");
  const { registerId } = await params;
  if (!canAccessRegister(authorization, registerId)) notFound();
  const [register, categories] = await Promise.all([
  ...
  return <PageContainer>
    <PageHeader eyebrow="Restaurant register" title={`${register.name} menu`} description="Build menu items from inventory servings. Sales consume the earliest-expiring ingredient batches first." actions={can(authorization, "REGISTERS_VIEW") ? <Link prefetch={false} href={`/registers?register=${register.id}`} className="flex h-10 items-center rounded-lg border border-border bg-card px-4 text-xs font-semibold">Back to register</Link> : null} />
    ...
    {can(authorization, "MENU_ITEM_CREATE") ? <details className="rounded-xl border border-border bg-card p-4"><summary className="cursor-pointer text-sm font-semibold">Add menu item</summary><div className="mt-4"><MenuItemForm registerId={register.id} products={productOptions} categories={categories} /></div></details> : null}
    <div className="grid gap-4">
      {register.menuItems.length ? register.menuItems.map((item) => <Surface key={item.id} className={cn("grid gap-4 p-5", !item.active && "opacity-60")}>
        <header className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] uppercase tracking-wider text-muted-foreground">{item.category} · {item.active ? "Active" : "Archived"}</p><h2 className="mt-1 text-lg font-semibold">{item.name}</h2><p className="mt-1 font-mono text-sm">{formatMvr(item.retailPriceLaari)}</p></div>{can(authorization, "MENU_ITEM_UPDATE") ? <form action={toggleMenuItemAction.bind(null, item.id, register.id, !item.active)}><button className="h-9 rounded-lg border border-border px-3 text-xs">{item.active ? "Archive" : "Activate"}</button></form> : null}</header>
        <div className="flex flex-wrap gap-2">{item.ingredients.map((ingredient) => <span key={ingredient.productId} className="rounded-full bg-accent px-3 py-1.5 text-[11px]">{ingredient.product.name} · {ingredient.servingMultiplier} × {formatQuantity(ingredient.product, measuredPerServing(ingredient.product))}{ingredient.standalone ? " · standalone" : ""}</span>)}</div>
        {can(authorization, "MENU_ITEM_UPDATE") ? <details><summary className="cursor-pointer text-xs font-semibold">Edit item and recipe</summary><div className="mt-3"><MenuItemForm registerId={register.id} products={productOptions} categories={categories} item={{ id: item.id, name: item.name, category: item.category, retailPrice: (item.retailPriceLaari / 100).toFixed(2), ingredients: item.ingredients.map((ingredient) => ({ productId: ingredient.productId, servingMultiplier: ingredient.servingMultiplier, standalone: ingredient.standalone })) }} /></div></details> : null}
