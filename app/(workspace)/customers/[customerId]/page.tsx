...
export default async function CustomerDetailPage({
  ...
  const mergeAccounts = authorization.user.isSiteAdmin ? await prisma.customer.findMany({ where: { active: true }, select: { id: true, name: true, updatedAt: true, creditLimitLaari: true, creditBills: { where: { status: "OUTSTANDING" }, select: { totalLaari: true } } } }) : [];
  const mergeOptions = mergeAccounts.map(c => ({ id: c.id, name: c.name, updatedAt: c.updatedAt.toISOString(), creditLimitLaari: c.creditLimitLaari, outstandingLaari: c.creditBills.reduce((n,b) => n+b.totalLaari,0) }));
  const mergeSource = mergeOptions.find(c => c.id === customer.id);
    ...
    <PageContainer>
      ...
      <section className="grid gap-3.5 xl:grid-cols-[minmax(0,0.85fr)_minmax(0,1.5fr)]">
        <div className="grid content-start gap-3.5">
          ...
          {mergeSource && <MergeCustomerForm source={mergeSource} candidates={mergeOptions.filter(c => c.id !== customer.id)} />}
          {canEditProfile || canEditLimit ? (
            <details id="edit-account" open={editing} className="scroll-mt-5 rounded-xl border border-border bg-card p-5">
              ...
              {canEditProfile ? (
              <MutationForm action={updateCustomerAction.bind(null, customer.id)} className="mt-4 grid gap-3">
              ...
              {canEditLimit ? <MutationForm action={updateCustomerCreditLimitAction.bind(null, customer.id)} className="mt-4 grid gap-3 border-t border-border pt-4"><CreditReview defaultValue={(customer.creditLimitLaari / 100).toFixed(2)} /><button type="submit" className="h-10 rounded-lg border border-border px-4 text-xs font-semibold">Save credit limit</button></MutationForm> : null}
            </details>
          ) : null}
        </div>
